...
seoTitle: "Claude Code만 쓰던 프로젝트에 Codex를 넣어본 이유와 운영 모델 변화"
created: "2026-04-15"
updated: "2026-04-15"
type: topic
sources: ["[[sources/2026-04-15-ai-frontier-ep86]]"]
tags: ["claude-code", "codex", "agentic-workflow", "harness", "workflow", "llm-ops"]
status: active
published: true
slug: "claude-code-to-codex"
description: "Claude Code만으로 굴리던 프로젝트에 Codex를 넣으면서, 도구 비교가 아니라 운영 모델 자체를 다시 설계하게 된 과정."
...
Claude Code만 쓰던 프로젝트에 [[entities/codex|Codex]]를 넣어봤다.
...
최근 들어 [[entities/claude-code|Claude Code]]를 쓰는 감각이 전과 조금 달라졌기 때문이다.
...
`/effort low | medium | high`로 thinking depth를 조절할 수 있게 된 것도 흥미롭긴 했다. 하지만 사용자 입장에서는 "좋아졌다"보다 "이제 내가 더 많이 조절해야 한다"는 감각으로 들어왔다.
...
### `.claude`만 있던 프로젝트에 `.codex`를 추가했다
...
결국 ECC나 superpowers에서 가져온 건 기능이 아니라 **작업 습관**이었다.
...
문제는 "Claude가 낫냐, Codex가 낫냐"가 아니었다. 문제는 **내 프로젝트가 어떤 조종석에만 맞춰져 있느냐**였다.
