...
- 이 문서는 최상위 워크스페이스 구조를 확정하던 시점의 검수 시트와 판단 요약을 남기는 `history retained here` 문서다.
- 현재 detached repo 운영 규칙의 `operational source`는 각 저장소 내부 문서를 우선 기준으로 본다.
- 따라서 이 문서는 현재 운영 설명서라기보다, 폴더 구조가 왜 이렇게 결정되었는지 추적하는 검수 메모로 사용한다.
...
운영 판단이 필요하면 detached repo 문서를 먼저 읽는다.

1. `shared-agent-harness/README.md`
2. `shared-agent-harness/docs/operating/source-of-truth-map.md`
3. `company-wiki/README.md`
4. `company-assistant-ops/README.md`

설계 배경과 판단 순서를 확인할 때만 아래 upstream 문서를 본다.
