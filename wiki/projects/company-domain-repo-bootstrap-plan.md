...
- `company-wiki`, `company-assistant-ops`, `shared-agent-harness` 폴더와 최소 문서 세트는 생성 완료
- `company-wiki`, `company-assistant-ops`는 `git init` 전이므로 `bootstrapped but not operational`
- `shared-agent-harness`는 1차 이주와 최소 executable surface까지 완료
- `company-wiki`는 `docs/operating/*`까지 보강되어 detached authoring operational source로 승격 중이다
- `company-assistant-ops`는 `README/AGENTS/CLAUDE/ARCHITECTURE/docs/operating/*` 세트로 detached execution operational source를 설명하는 구조까지 보강됐다
