...
- 이 문서는 `즉시 운영 구조`가 어떤 판단에서 나왔는지 설명하는 `history retained here` 문서다.
- 공용 role/lane 운영 원칙의 현재 detached `operational source`는 `shared-agent-harness` 저장소 문서를 우선 기준으로 본다.
- 특히 다음 문서를 우선 참조한다.
