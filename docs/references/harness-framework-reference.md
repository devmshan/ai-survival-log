이 문서는 Harness 프레임워크의 참조 메모다.

현재 `ai-survival-log`와 `ai-survival-log-site`는 이 문서의 아이디어를 선택적으로 차용하지만, 여기 적힌 실행기 메커니즘(`phases/*`, `scripts/execute.py`)을 현재 운영 계약으로 사용하지는 않는다.

실제 적용 기준은 각 저장소의 `AGENTS.md`, `ARCHITECTURE.md`, `docs/operating/*`, `docs/templates/prd.md`, `docs/adr/*`를 우선한다.

아래 내용은 단계 설계와 workflow 아이디어를 참고할 때만 사용한다.
